...
def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
                    ...
                    set_training_mode=True, amp_autocast=suppress):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        samples = samples.to(device, non_blocking=True)  # B, N_view, 3, 224, 224
...
def evaluate(data_loader, model, device):
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            y_pred.extend(torch.max(output, 1)[1].data.cpu().numpy())  # save prediction
            y_true.extend(target.data.cpu().numpy())  # save GT
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, y_true, y_pred
