...
class MixerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.shift = Shift(dim)#Hand(dim)#Shift(dim)
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path(self.shift(self.norm1(x)))
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(self.ls1 * self.shift(self.norm1(x)))
...
class MlpMixer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.blocks(x)
        x = self.norm(x)
        x = x.reshape(B//self.view_number, self.view_number, N, C)  # B, V, WH, C
        x, _ = x.max(1)  # view
        x = x.mean(dim=1)  # patch
        x = self.head(x)
        return x
